...
logFile<-'/Users/lptolik/Documents/Projects/ShapleyDataR/FBAfilesReportBM.log'
...
idxRe<-which(uppbnd(modBM)-lowbnd(modBM)&!grepl('^(pengOUT|bmOUT)$',modBM@react_id))
reId<-modBM@react_id[idxRe]
res<-fbaShapley(modBM,reId = reId,logName = logFile)
# N<-modBM@react_num
# convTol<-0.1
# tol<-0.01
# phi<-list()
...
#   if(t%%10==0){
#     save(phi,t,N,vTot,v,val,permL,sd,perfTolerance,vNull,tolV,modBM,file = 'fbaShapleyBM.RData')
#   }
#   }
#   perm<-makePerm(N)
#   v<-rep(0.0,N)
...
# }
#
# save.image(file='fba.shapley.bm.rdata')
